...
export const Booking: React.FC = () => {
  ...
  const [showAllServices, setShowAllServices] = useState(true); // Mostrar todos por padrão
    ...
    const loadData = async () => {
      try {
        ...
        try {
          ...
          // Garantir que shopId esteja preenchido
  ...
  const isBlocked = (date: string, time: string, barberId: string) => {
    ...
    // ðŸŽ¨ UX: Desabilitar horários passados no dia atual (visual apenas)
  ...
  const handleConfirm = async () => {
    ...
    if (selectedServices.length === 0) {
      addNotification('error', 'Selecione pelo menos um serviço');
      return;
    }
    ...
    try {
      ...
      // DEBUG: Verificar o que tem dentro do JWT
      const token = localStorage.getItem('accessToken');
      if (token) {
        console.group('ðŸ” DEBUG JWT ANTES DE CRIAR APPOINTMENT');
        try {
          const payload = JSON.parse(atob(token.split('.')[1]));
        } catch (e) {
          console.error('âŒ Erro ao decodificar JWT:', e);
        }
        console.groupEnd();
      }
      ...
      if (error?.statusCode === 403 || error?.status === 403) {
        addNotification(
          'error',
          'Você só pode agendar para seu próprio perfil. Contate o administrador se precisar de ajuda.',
          'Acesso Negado'
        );
        return;
      }
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 pb-32">
        <div className="bg-white dark:bg-gray-800 rounded-[40px] shadow-2xl border dark:border-gray-700 overflow-hidden min-h-[550px] flex items-center justify-center">
          <div className="text-center p-8">
            ...
            <button onClick={() => navigate('/')} className="bg-amber-500 text-white px-8 py-3 rounded-[22px] font-black uppercase text-xs tracking-wider">Voltar ao Início</button>
          </div>
        </div>
      </div>
    );
      ...
      <div className="bg-white dark:bg-gray-800 rounded-[40px] shadow-2xl border dark:border-gray-700 overflow-hidden min-h-[550px]">
                ...
                <button key={s.id} onClick={() => setSelectedServices(prev => prev.includes(s.id) ? prev.filter(id => id !== s.id) : [...prev, s.id])} className={`p-6 rounded-[30px] border-2 transition-all flex items-center gap-5 text-left group ${selectedServices.includes(s.id) ? 'border-amber-500 bg-amber-50 dark:bg-amber-900/20' : 'border-gray-50 dark:border-gray-700 hover:border-amber-500/30'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 text-xs">
                      <span className="text-amber-600 font-black">R$ {s.price.toFixed(2)}</span>
          ...
          <div className="p-8 md:p-12 animate-fade-in">
            ...
            <div className="relative mb-8">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-4 rounded-[24px] border-2 border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-white focus:border-amber-500 outline-none transition-all"
              />
            </div>
              ...
              {allClients
                ...
                .map(c => (
                  <button
                    ...
                    className={`p-6 rounded-[30px] border-2 transition-all group ${selectedClient === c.id ? 'border-amber-500 bg-amber-50 dark:bg-amber-900/20' : 'border-gray-200 dark:border-gray-700 hover:border-amber-500/30'}`}
                  >
                    <div className="w-16 h-16 rounded-[22px] mx-auto mb-3 flex items-center justify-center bg-gray-100 dark:bg-gray-700 text-gray-400 group-hover:bg-amber-500 group-hover:text-white transition-all">
                      <User size={28} />
                    </div>
            ...
            <div className="mt-12 flex justify-between">
              <button onClick={() => setStep(1)} className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 hover:text-amber-500 transition-colors">Voltar</button>
              <button
                disabled={!selectedClient}
                onClick={() => setStep(3)}
                className="bg-amber-500 text-white px-12 py-5 rounded-[22px] font-black uppercase text-[11px] tracking-[0.2em] shadow-xl shadow-amber-500/30 disabled:opacity-30 transition-all active:scale-95"
              >
                Continuar
              </button>
            </div>
        ...
        {step === 3 && user && (user.role === 'ADMIN' || user.role === 'SUPER_ADMIN') && (
          <div className="p-8 md:p-12 animate-fade-in text-center">
            ...
            <div className="mt-16 flex justify-between px-4"><button onClick={() => setStep(2)} className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 hover:text-amber-500 transition-colors">Voltar</button><button disabled={!selectedBarber} onClick={() => setStep(4)} className="bg-amber-500 text-white px-12 py-5 rounded-[22px] font-black uppercase text-[11px] tracking-[0.2em] shadow-xl shadow-amber-500/20 active:scale-95 transition-all">Continuar</button></div>
          </div>
        )}
            ...
            <div className="text-center mb-10">
              ...
              {totalDuration > 0 && (
                <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mt-4 flex items-center justify-center gap-2">
                  <Clock size={14} className="text-amber-500" /> Duração total: {totalDuration} minutos
                </p>
              )}
            </div>
        ...
        {step === 4 && user && (user.role === 'ADMIN' || user.role === 'SUPER_ADMIN') && (
          <div className="p-8 md:p-12 animate-fade-in bg-[#111827]">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              <div className="h-full max-h-[450px] overflow-y-auto pr-2 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-amber-500/30 grid grid-cols-2 gap-3 content-start">
                {timeSlots.map(time => {
            ...
            <div className="mt-16 flex justify-between"><button onClick={() => setStep(3)} className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 hover:text-amber-500">Voltar</button><button disabled={!selectedTime} onClick={() => setStep(5)} className="bg-amber-500 text-white px-12 py-5 rounded-[22px] font-black uppercase text-[11px] tracking-[0.2em] shadow-xl">Revisar</button></div>
          </div>
        )}

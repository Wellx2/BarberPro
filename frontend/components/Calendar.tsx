...
export const Calendar: React.FC<CalendarProps> = ({ selectedDate, onDateSelect, isDateDisabled }) => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (isSelected) {
          ...
          buttonClass += "bg-amber-500 text-white shadow-md transform scale-110 font-bold z-10 ";
      } else if (isDisabled) {
          ...
          // Ajustado: z-0 para itens normais
          buttonClass += "text-gray-700 dark:text-gray-200 hover:bg-amber-100 dark:hover:bg-amber-900/30 hover:text-amber-700 dark:hover:text-amber-400 cursor-pointer z-0 ";
          if (isToday) buttonClass += "border border-amber-500 font-bold ";
      }
